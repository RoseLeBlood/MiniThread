#ifndef _MINLIB_AUTO_VERSION_H_
#define _MINLIB_AUTO_VERSION_H_

namespace mn{
	
	//Software Status
	#define MN_VERSION_STATUS  "Beta"
	#define MN_VERSION_STATUS_SHORT  "b"
	
	//Standard Version Type
	#define MN_VERSION_MAJOR  2
	#define MN_VERSION_MINOR  29
	#define MN_VERSION_BUILD  8906
	#define MN_VERSION_REVISION  10
	
	//Miscellaneous Version Types
	#define MN_VERSION_BUILDS_COUNT  53
	#define MN_VERSION_RC_FILEVERSION 2,29,8906,10
	#define MN_VERSION_RC_FILEVERSION_STRING "2, 29, 8906, 10\0"
	#define MN_VERSION_FULLVERSION_STRING  "2.29.8906.10"
	
	//These values are to keep track of your versioning state, don't modify them.
	#define MN_VERSION_BUILD_HISTORY  0
	

}
#endif //_MINLIB_AUTO_VERSION_H_
