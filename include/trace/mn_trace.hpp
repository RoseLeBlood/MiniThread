...
namespace mn {
    namespace trace {
        ...
        public:
            ...
            tracer& instance() {
                if(m_pInstance == NULL) m_pInstance = new tracer(); 
                return m_pInstance;
            }
        ...
        #define traceCREATE_MUTEX( pxMutex ) \
            tracer::instance().add_mutex_item(pxMutex, semaphore_trace_state::StateCreate)

        #undef traceGIVE_MUTEX_RECURSIVE
        #define traceGIVE_MUTEX_RECURSIVE( pxMutex ) \
            tracer::instance().add_mutex_item(pxMutex, semaphore_trace_state::StateExitLock)

        #undef traceGIVE_MUTEX_RECURSIVE_FAILED
        #define traceGIVE_MUTEX_RECURSIVE_FAILED( pxMutex ) \
            tracer::instance().add_mutex_item(pxMutex, semaphore_trace_state::StateFailedExitLock)

        #undef traceTAKE_MUTEX_RECURSIVE
        #define traceTAKE_MUTEX_RECURSIVE( pxMutex ) \
            tracer::instance().add_mutex_item(pxMutex, semaphore_trace_state::StateEnterLock)

        #undef traceTAKE_MUTEX_RECURSIVE_FAILED
        #define traceTAKE_MUTEX_RECURSIVE_FAILED( pxMutex ) \
            tracer::instance().add_mutex_item(pxMutex, semaphore_trace_state::StateFailedLock)
