...
public:
    ...
    using base_t = basic_mempool_vector<TType, nElements, TALLOCATOR>;
    /**
     * Ctor 
     */ 
     ...
     */ 
    virtual type_t* allocate(unsigned long xTicksToWait = portMAX_DELAY) {
     ...
     */ 
    virtual bool  free(type_t* mem, bool* wasCurropted, unsigned long xTicksToWait = portMAX_DELAY) {
     ...
     */ 
    unsigned int get_used() { return get<vmempool_chunk_state::Used>(); }
    /**
     * How many elements are marked as free
     * @return The number of elements / chunks are marked as free
     */ 
    unsigned int get_free() { return get<vmempool_chunk_state::Free>(); }
    /**
     * How many elements are marked as blocked
     * @return The number of elements / chunks are marked as blocked
     */ 
    unsigned int get_blocked() { return get<vmempool_chunk_state::Blocked>(); }

    template<vmempool_chunk_state state> int get() {
        /*automutx_t lock(m_mutex);*/ 
        unsigned int _ret = 0;
